...
test('root account creates sub-account', async () => {
    ...
    const { transaction } = await rootAccount.createAccount(newAccountId, key.getPublicKey(), 0n);

    await rootAccount.provider.viewTransactionStatus(transaction.hash, rootAccount.accountId, 'FINAL');
...
test('account can send meta transaction with Transfer', async () => {
    ...
    const childAccount = new Account(newAccountId, rootAccount.provider, new KeyPairSigner(keypair));

    const [, signedDelegate] = await childAccount.createSignedMetaTransaction(childAccount.accountId, [
...
test('account can send meta transaction with DeployGlobalContract(AccountId)', async () => {
    ...
    const [, signedDelegate] = await childAccount.createSignedMetaTransaction(childAccount.accountId, [
        actionCreators.deployGlobalContract(new Uint8Array(128), new GlobalContractDeployMode({ AccountId: null })),
    ]);
...
test('account can send meta transaction with DeployGlobalContract(CodeHash)', async () => {
    ...
    const [, signedDelegate] = await childAccount.createSignedMetaTransaction(childAccount.accountId, [
        actionCreators.deployGlobalContract(new Uint8Array(128), new GlobalContractDeployMode({ CodeHash: null })),
    ]);
...
test(
    ...
    async () => {
        const newAccountId = `${Date.now()}.${rootAccount.accountId}`;
        const keypair = KeyPair.fromRandom('ed25519');
        await rootAccount.createAccount(newAccountId, keypair.getPublicKey(), NEAR.toUnits('1'));

        const account = new Account(newAccountId, rootAccount.provider, new KeyPairSigner(keypair));
