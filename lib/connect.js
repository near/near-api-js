    ...
    if (config.keyPath && (config.keyStore || config.deps && config.deps.keyStore)) {
        try {
            ...
            if (accountKeyFile[0]) {
                ...
                logger_1.getLogger().debug(`Loaded master account ${accountKeyFile[0]} key from ${config.keyPath} with public key = ${keyPair.getPublicKey()}`);
            }
        }
        catch (error) {
            logger_1.getLogger().error(`Failed to load master account key from ${config.keyPath}: ${error}`);
        }
    }
