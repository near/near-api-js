...
class WalletConnection {
    ...
    async requestSignIn(contractIdOrOptions = {}, title, successUrl, failureUrl) {
        ...
        if (typeof contractIdOrOptions === 'string') {
            logger_1.getLogger().deprecate('requestSignIn(contractId, title)', '`title` ignored; use `requestSignIn({ contractId, successUrl, failureUrl })` instead');
    ...
    async requestSignTransactions(...args) {
        if (Array.isArray(args[0])) {
            logger_1.getLogger().deprecate('WalletConnection.requestSignTransactions(transactions, callbackUrl, meta)', 'use `WalletConnection.requestSignTransactions(RequestSignTransactionsOptions)` instead');
