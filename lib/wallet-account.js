...
class WalletConnection {
     ...
     * ```
     */
    async batchSendTransactions(transactionOptions, callbackUrl) {
        ...
        const transactions = await Promise.all(transactionOptions.map(async ({ receiverId, actions }, index) => {
            ...
            return transaction_1.createTransaction(accountId, publicKey, receiverId, nonce, actions, blockHash);
        }));
