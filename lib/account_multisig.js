...
exports.AccountMultisig = exports.modIfMultisig = exports.MULTISIG_CONFIRM_METHODS = exports.MULTISIG_VIEW_METHODS = exports.MULTISIG_CHANGE_METHODS = exports.MULTISIG_DEPOSIT = exports.MULTISIG_GAS = exports.MULTISIG_ALLOWANCE = exports.MULTISIG_STORAGE_KEY = void 0;
...
********************************/
exports.modIfMultisig = (near, options) => {
    near.account = async (accountId) => {
        const account = new account_1.Account(near.connection, accountId);
        await account.state();
        const localKey = (await near.connection.signer.getPublicKey(account.accountId, near.connection.networkId)).toString();
        const accessKeys = await account.getAccessKeys();
        const hasFAK = accessKeys.find((k) => k.public_key === localKey &&
            k.access_key.permission &&
            k.access_key.permission === 'FullAccess');
        if (hasFAK) {
            return account;
        }
        const use2fa = accessKeys.find((k) => k.public_key === localKey &&
            k.access_key.permission &&
            k.access_key.permission.FunctionCall &&
            k.access_key.permission.FunctionCall.method_names.some((mn) => exports.MULTISIG_CHANGE_METHODS.includes(mn)));
        if (use2fa) {
