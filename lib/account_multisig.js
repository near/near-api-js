...
class Account2FA extends AccountMultisig {
    ...
    async deployMultisig(contractBytes, newLocalPublicKey) {
        const { accountId } = this;
        let [confirmOnlyKey, localPublicKey, recoveryMethods, accessKeys,] = await Promise.all([
        ...
        confirmOnlyKey = key_pair_1.PublicKey.from(confirmOnlyKey.publicKey);
        ...
        const fak2lak = accessKeys
            .filter(({ public_key, access_key: { permission } }) => public_key !== localPublicKeyStr &&
            permission === 'FullAccess' &&
            !seedOrLedgerKey.includes(public_key))
            .map((ak) => key_pair_1.PublicKey.from(ak.public_key));
        const lak = transaction_1.functionCallAccessKey(accountId, exports.MULTISIG_CHANGE_METHODS, null);
        const actions = [
            transaction_1.addKey(confirmOnlyKey, transaction_1.functionCallAccessKey(accountId, exports.MULTISIG_CONFIRM_METHODS, null)),
            transaction_1.deleteKey(localPublicKey),
            transaction_1.addKey(newLocalPublicKey, lak),
            ...fak2lak.map((pk) => transaction_1.deleteKey(pk)),
            ...fak2lak.map((pk) => transaction_1.addKey(pk, lak)),
    ...
    async disable(contractBytes, newLocalPublicKey) {
        const { accountId } = this;
        let [confirmOnlyKey, localPublicKey, accessKeys] = await Promise.all([
            this.postSignedJson('/2fa/getAccessKey', { accountId }),
            this.connection.signer.getPublicKey(accountId, this.connection.networkId),
            this.getAccessKeys()
        ]);
        confirmOnlyKey = key_pair_1.PublicKey.from(confirmOnlyKey.publicKey);
        const localPublicKeyStr = localPublicKey.toString();
        const lak2fak = accessKeys.filter(({ public_key, access_key }) => public_key !== localPublicKeyStr &&
        ...
        const actions = [
            ...
            transaction_1.deleteKey(localPublicKey),
            transaction_1.addKey(newLocalPublicKey, null),
