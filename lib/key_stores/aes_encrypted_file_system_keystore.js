...
exports.AESEncryptedFileSystemKeyStore = exports.readKeyFile = exports.loadJsonFile = void 0;
...
/** @hidden */
async function readKeyFile(filename, password) {
    ...
    return [accountInfo.account_id, key_pair_1.KeyPair.fromString(crypto_js_1.default.AES.decrypt(privateKey, password).toString(crypto_js_1.default.enc.Utf8))];
}
exports.readKeyFile = readKeyFile;
/**
 * This module contains the {@link AESEncryptedFileSystemKeyStore} class which is used to store keys on the file system.
 *
 ...
 *
 * const keyStore = new keyStores.AESEncryptedFileSystemKeyStore(`${homedir()}/.near-credentials`);
 * const config = {
 *   keyStore, // instance of AESEncryptedFileSystemKeyStore
 *   networkId: 'testnet',
...
class AESEncryptedFileSystemKeyStore extends keystore_1.KeyStore {
    ...
    async setKey(networkId, accountId, keyPair) {
        ...
        const content = { account_id: accountId, public_key: keyPair.getPublicKey().toString(), private_key: crypto_js_1.default.AES.encrypt(keyPair.toString(), this.password).toString() };
    ...
    async getKey(networkId, accountId) {
        ...
        const accountKeyPair = await readKeyFile(this.getKeyFilePath(networkId, accountId), this.password);
    ...
    /** @hidden */
    toString() {
        return `AESEncryptedFileSystemKeyStore(${this.keyDir})`;
    }
}
exports.AESEncryptedFileSystemKeyStore = AESEncryptedFileSystemKeyStore;
