...
class Account {
    ...
    /** @hidden */
    get ready() {
        logger_1.getLogger().deprecate('Account.ready()', 'not needed anymore, always ready');
        return Promise.resolve();
    }
    async fetchState() {
        logger_1.getLogger().deprecate('Account.fetchState()', 'use `Account.state()` instead');
    }
    /**
    ...
    printLogsAndFailures(contractId, results) {
        for (const result of results) {
            logger_1.getLogger().info(`Receipt${result.receiptIds.length > 1 ? 's' : ''}: ${result.receiptIds.join(', ')}`);
            this.printLogs(contractId, result.logs, '\t');
            if (result.failure) {
                logger_1.getLogger().warn(`\tFailure [${contractId}]: ${result.failure}`);
            }
        }
    }
    /** @hidden */
    printLogs(contractId, logs, prefix = '') {
        for (const log of logs) {
            logger_1.getLogger().info(`${prefix}Log [${contractId}]: ${log}`);
        }
    }
    /**
    ...
    signAndSendTransactionV1(receiverId, actions) {
        logger_1.getLogger().deprecate('Account.signAndSendTransaction(receiverId, actions', 'use `Account.signAndSendTransaction(SignAndSendTransactionOptions)` instead');
        ...
        const result = await exponential_backoff_1.default(TX_NONCE_RETRY_WAIT, TX_NONCE_RETRY_NUMBER, TX_NONCE_RETRY_WAIT_BACKOFF, async () => {
            ...
            try {
                ...
                if (error.type === 'InvalidNonce') {
                    logger_1.getLogger().warn(`Retrying transaction ${receiverId}:${borsh_1.baseEncode(txHash)} with new nonce.`);
    ...
    functionCallV1(contractId, methodName, args, gas, amount) {
        logger_1.getLogger().deprecate('Account.functionCall(contractId, methodName, args, gas, amount)', 'use `Account.functionCall(FunctionCallOptions)` instead');
        args = args || {};
