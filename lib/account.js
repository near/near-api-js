...
class Account {
    ...
    async stake(publicKey, amount) {
        const status = await this.connection.provider.status();
        if (compare_versions_1.default(status.version.version, '0.4.0') == -1) {
            throw new Error(`Staking for nearcore version under 0.4.0 is prohibited. Current nearcore version ${status.version.version}`);
        }
        return this.signAndSendTransaction(this.accountId, [transaction_1.stake(amount, key_pair_1.BlsPublicKey.from(publicKey))]);
    }
