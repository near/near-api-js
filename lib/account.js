...
class Account {
    ...
    async signAndSendTransaction(receiverId, actions) {
        ...
        const [txHash, signedTx] = await transaction_1.signTransaction(receiverId, ++this._access_key.nonce, actions, this.connection.signer, this.accountId, this.connection.networkId);
        let result;
        try {
    ...
    async addKey(publicKey, contractId, methodName, balanceOwner, amount) {
        ...
        if (contractId === null) {
            ...
            accessKey = transaction_1.functionCallAccessKey(contractId, methodName === null ? [] : [methodName], amount);
        }
