...
class Account {
     ...
     */
    async functionCall({ contractId, methodName, args = {}, gas = constants_1.DEFAULT_FUNCTION_CALL_GAS, attachedDeposit, walletMeta, walletCallbackUrl, stringify, jsContract }) {
     ...
     * @returns {Promise<any>}
     */
    async viewFunction(contractId, methodName, args = {}, { parse = parseJsonFromRawResponse, stringify = bytesJsonStringify, jsContract = false, blockQuery = { finality: 'optimistic' } } = {}) {
        let encodedArgs;
        this.validateArgs(args);
        if (jsContract) {
    ...
    createTransaction(recevier) {
        return new Transaction(this, recevier);
    }
    async hasDeployedContract() {
        return (await this.state()).code_hash === constants_1.EMPTY_CONTRACT_HASH;
    }
}
...
class Transaction {
    ...
    addKey(publicKey, accessKey = transaction_1.fullAccessKey()) {
        this.actions.push(transaction_1.addKey(key_pair_1.PublicKey.from(publicKey), accessKey));
        return this;
    }
    createAccount() {
        this.accountToBeCreated = true;
        this.actions.push(transaction_1.createAccount());
        return this;
    }
    deleteAccount(beneficiaryId) {
        this.actions.push(transaction_1.deleteAccount(beneficiaryId));
        return this;
    }
    deleteKey(publicKey) {
        this.actions.push(transaction_1.deleteKey(key_pair_1.PublicKey.from(publicKey)));
        return this;
    }
    deployContract(code) {
        this.actions.push(transaction_1.deployContract(code));
        return this;
    }
    functionCall(methodName, args, { gas = constants_1.DEFAULT_FUNCTION_CALL_GAS, attachedDeposit = _1.NEAR.from(0), } = {}) {
        this.actions.push(transaction_1.functionCall(methodName, args, misc_1.parseGas(gas), misc_1.parseNEAR(attachedDeposit)));
        return this;
    }
    stake(amount, publicKey) {
        this.actions.push(transaction_1.stake(new bn_js_1.default(amount), key_pair_1.PublicKey.from(publicKey)));
        return this;
    }
    transfer(amount) {
        this._transferAmount = misc_1.parseNEAR(amount);
        this.actions.push(transaction_1.transfer(this._transferAmount));
        return this;
    }
    get accountCreated() {
    ...
    get transferAmount() {
        return this._transferAmount ?? _1.NEAR.from('0');
    }
    signAndSend(keyPair) {
