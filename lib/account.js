    ...
    printLogsAndFailures(contractId, results) {
        if (process.env["NEAR_RUNNER_DEBUG"]) {
            for (const result of results) {
                console.error(`Receipt${result.receiptIds.length > 1 ? 's' : ''}: ${result.receiptIds.join(', ')}`);
    ...
    printLogs(contractId, logs, prefix = '') {
        if (process.env["NEAR_RUNNER_DEBUG"]) {
            for (const log of logs) {
                console.error(`${prefix}Log [${contractId}]: ${log}`);
            }
        }
    }
    /**
        ...
        const result = await exponential_backoff_1.default(TX_NONCE_RETRY_WAIT, TX_NONCE_RETRY_NUMBER, TX_NONCE_RETRY_WAIT_BACKOFF, async () => {
            ...
            try {
                ...
                if (error.type === 'Expired') {
                    console.warn(`Retrying transaction ${receiverId}:${borsh_1.baseEncode(txHash)} due to expired block hash`);
                    return null;
                }
