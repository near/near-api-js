...
async function fetchJson(connectionInfoOrUrl, json) {
        ...
        try {
            ...
            if (!response.ok) {
                if (response.status === 503) {
                    errors_1.logWarning(`Retrying HTTP request for ${nextUrl} as ${currentUrl} not available now`);
                    return null;
                }
            ...
            if (error.toString().includes('FetchError') || error.toString().includes('Failed to fetch')) {
                errors_1.logWarning(`Retrying HTTP request for ${nextUrl} because of error: ${error}`);
                return null;
            }
            throw error;
        }
    ...
    if (!response) {
        throw new providers_1.TypedError(`Exceeded ${RETRY_NUMBER} attempts.`, 'RetriesExceeded');
    }
