        ...
        try {
            ...
            if (!response.ok) {
                if (response.status === 503) {
                    logger_1.getLogger().debug(`Retrying HTTP request for ${url} as it's not available now`);
                    return null;
                }
            ...
            if (error.toString().includes('FetchError') || error.toString().includes('Failed to fetch')) {
                logger_1.getLogger().debug(`Retrying HTTP request for ${url} because of error: ${error}`);
                return null;
            }
            throw error;
        }
