...
class JsonRpcProvider extends provider_1.Provider {
    ...
    async block(blockQuery) {
        ...
        if (typeof blockQuery !== 'object') {
            logger_1.getLogger().deprecate('JsonRpcProvider.block(blockId)', 'use `block({ blockId })` or `block({ finality })` instead');
            blockId = blockQuery;
        }
    ...
    async experimental_genesisConfig() {
        logger_1.getLogger().deprecate('JsonRpcProvider.experimental_protocolConfig({ sync_checkpoint: \'genesis\' })', 'use `experimental_protocolConfig` to fetch the up-to-date or genesis protocol config explicitly');
    ...
    async experimental_lightClientProof(request) {
        logger_1.getLogger().deprecate('JsonRpcProvider.experimental_lightClientProof(request)', 'use `lightClientProof` instead');
        ...
        const result = await exponential_backoff_1.default(REQUEST_RETRY_WAIT, REQUEST_RETRY_NUMBER, REQUEST_RETRY_WAIT_BACKOFF, async () => {
            try {
                ...
                if (error.type === 'TimeoutError') {
                    logger_1.getLogger().debug(`Retrying request to ${method} as it has timed out`, params);
                    return null;
                }
                throw error;
            }
