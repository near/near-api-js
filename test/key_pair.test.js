...
describe('Using Secp256k1 Curve', () => {
    test('Should sign and verify with Secp256k1', async () => {
        const keyPair = new nearApi.utils.key_pair.KeyPairSecp256k1('x7uezeY3mMFdEmRGvnNgPiXxvsHJ2kzEtE1AdBcgqLD');
        expect(keyPair.publicKey.toString()).toEqual('secp256k1:vpNuvJMTFRbNJ8wDRCwwMHGfF7derNoHgmCeuf4xmjDw');
        const message = new Uint8Array(sha256.array('message'));
        const signature = keyPair.sign(message);
        expect(nearApi.utils.serialize.base_encode(signature.signature)).toEqual('mDEAFfRrJQ8ZimqTUuhCAue6iR75yxDWwdLuRNqeNt2f3y5XY1p43AQh3VTwuGmT99cxo4GLACuAAukEZb9fzMU');
    });
    ...
    test('Should sign and verify public key created using Secp256k1', async () => {
        const keyPair = new nearApi.utils.key_pair.KeyPairSecp256k1('4ZYmV7Be2TEvwrcxEqUdQLwwR8BTaauWKfQAh7WYoVdC');
        const message = new Uint8Array(sha256.array('message'));
        const signature = keyPair.sign(message);
        const publicKey = nearApi.utils.key_pair.PublicKey.from('secp256k1:zrrXFcCxRJCMzuPhCWrgyt4R1W5vDYer5iYyKauSX72X');
        expect(publicKey.verify(message, signature.signature)).toBeTruthy();
    });

    test('Should create proper publicKey from secret using Secp256k1', async () => {
        const keyPair = new nearApi.utils.key_pair.KeyPairSecp256k1('4ZYmV7Be2TEvwrcxEqUdQLwwR8BTaauWKfQAh7WYoVdC');
        expect(keyPair.publicKey.toString()).toEqual('secp256k1:zrrXFcCxRJCMzuPhCWrgyt4R1W5vDYer5iYyKauSX72X');
    });
