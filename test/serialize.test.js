...
test('serialize and sign multi-action tx', async() => {
    ...
    const blockHash = borsh.baseDecode('244ZQ9cgj3CQ6bWBdytfrJMuMQ1jdXLFGnr4HhvtCTnM');
    ...
    expect(borsh.baseEncode(hash)).toEqual('Fo3MJ9XzKjnKuDuQKhDAC6fra5H2UWawRejFSEpPNk3Y');
    const serialized = borsh.serialize(nearApi.transactions.SCHEMA, transaction);
    ...
    for (const testFile of testFiles) {
        if (/.+\.json$/.test(testFile)) {
            ...
            test(testFile, () => {
                ...
                const deserialized = borsh.deserialize(nearApi.transactions.SCHEMA, type, data);
                const serialized = borsh.serialize(nearApi.transactions.SCHEMA, deserialized);
