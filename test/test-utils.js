...
// This key is only available for testing purposes when node is configured with the seed flags.
...
module.exports = {
    ...
    connect: async function (options = {}) {
        ...
        if (this.options.useDevAccount) {
            this.options.accountId = testAccountName;
    ...
    createAccountWithLocalNodeConnection: async function (newAccountName, newAccountPublicKey) {
        ...
        this.deps.keyStore.setKey(testAccountName, devKey); // need to have dev account in key store to use this.
        const createAccountResponse = await account.createAccount(newAccountName, newAccountPublicKey, 1000000000000, testAccountName);
