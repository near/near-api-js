...
test('create account and then view account returns the created account', async () => {
    ...
    const newAccount = new nearAPIJs.Account(nearjs.connection, newAccountName);
...
test('delete account', async() => {
    ...
    const reloaded = new nearAPIJs.Account(sender.connection, sender);
...
describe('with deploy contract', () => {
    ...
    beforeAll(async () => {
        ...
        contract = new nearAPIJs.Contract(workingAccount, contractId, {
    ...
    test('make function calls via account', async() => {
        ...
        expect(nearAPIJs.providers.getTransactionLastResult(result2)).toEqual(setCallValue);
    ...
    test('can have view methods only', async () => {
        const contract = new nearAPIJs.Contract(workingAccount, contractId, {
            viewMethods: ['hello'],
        });
