...
describe('rpc-errors', () => {
    ...
    test('test parse error object', async () => {
        ...
        const rpcError = parseRpcError(JSON.parse(errorStr).status.Failure);
        expect(rpcError).toEqual(new RpcError('{"index":0,"kind":{"EvmError":"ArgumentParseError"}}', 'type'));
    });

    test('test parseReceiptExecutionFailure', async () => {
        let result =
        {
            status: {
                Failure: {
        ...
        let error = parseReceiptExecutionFailure(result);
    ...
    test('test ReceiptExecutionFailure should have errorPath in context', async () => {
        ...
        let error1 = parseReceiptExecutionFailure(result);
        expect(error1.errorPath).toBe(errorPath);
