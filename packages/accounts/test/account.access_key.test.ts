...
test('remove access key no longer works', async () => {
    ...
    // Override in the key store the workingAccount key to the given access key.
    await nearjs.connection.signer.keyStore.setKey(
        networkId,
        nearjs.accountCreator.masterAccount.accountId,
        keyPair,
    );
    let failed = true;
    try {
        ...
        expect(e.message).toEqual(
            `Can not sign transactions for account ${nearjs.accountCreator.masterAccount.accountId} on network ${networkId}, no matching key pair exists for this account`,
        );
