...
['viewMethod', 'changeMethod'].forEach((method) => {
    describe(method, () => {
        ...
        test('throws PositionalArgsError if first argument is not an object', async () => {
            await expect(contract[method](1)).rejects.toBeInstanceOf(
                PositionalArgsError,
            );
            await expect(contract[method]('lol')).rejects.toBeInstanceOf(
                PositionalArgsError,
            );
            await expect(contract[method]([])).rejects.toBeInstanceOf(
                PositionalArgsError,
            );
            await expect(contract[method](new Date())).rejects.toBeInstanceOf(
                PositionalArgsError,
            );
            await expect(contract[method](null)).rejects.toBeInstanceOf(
                PositionalArgsError,
            );
            await expect(contract[method](new Set())).rejects.toBeInstanceOf(
                PositionalArgsError,
            );
        });
        ...
        test('throws PositionalArgsError if given too many arguments', () => {
            return expect(
                contract[method]({}, 1, 0, 'oops'),
            ).rejects.toBeInstanceOf(PositionalArgsError);
        });
...
describe('changeMethod', () => {
    test('throws error message for invalid gas argument', () => {
        return expect(
            contract.changeMethod({ a: 1 }, 'whatever'),
        ).rejects.toThrow(
    ...
    test('gives error message for invalid amount argument', () => {
        return expect(
            contract.changeMethod({ a: 1 }, 1000, 'whatever'),
        ).rejects.toThrow(
...
describe('local view execution', () => {
    ...
    test('calls get_messages() function using cache data', async () => {
        ...
        expect(contract.connection.provider.query).toHaveBeenCalled();
