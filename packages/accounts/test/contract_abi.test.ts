...
describe('add', () => {
    ...
    test('throws ArgumentSchemaError if required argument was not supplied', async () => {
        await expect(contract.add({})).rejects.toBeInstanceOf(
            ArgumentSchemaError,
        );
    });

    test('throws ArgumentSchemaError if argument has unexpected type', async () => {
        await expect(contract.add({ a: 1, b: [3, 4] })).rejects.toBeInstanceOf(
            ArgumentSchemaError,
        );
    });
...
describe('add_call', () => {
    ...
    test('throws ArgumentSchemaError if required argument was not supplied', async () => {
        await expect(contract.add_call({ args: {} })).rejects.toBeInstanceOf(
            ArgumentSchemaError,
        );
    });
    ...
    test('throws ArgumentSchemaError if argument has unexpected type', async () => {
        await expect(
            contract.add_call({ args: { a: 1, b: [3, 4] } }),
        ).rejects.toBeInstanceOf(ArgumentSchemaError);
    });
...
describe('Contract constructor', () => {
    test('throws UnsupportedSerializationError when ABI has borsh serialization', async () => {
        ...
        await expect(contract.add({ a: 1 })).rejects.toBeInstanceOf(
            UnsupportedSerializationError,
        );
    });
});
