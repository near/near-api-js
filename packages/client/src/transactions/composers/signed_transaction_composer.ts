...
export class SignedTransactionComposer extends TransactionComposer {
    ...
    async toSignedDelegateAction(transaction?: MetaTransactionOptions) {
        ...
        if (!maxBlockHeight) {
            const { header } = await this.account.provider.viewBlock({
                finality: 'optimistic',
            });
            const ttl =
                transaction?.blockHeightTtl ||
    ...
    private verifySigner(signingAccount: string) {
        if (signingAccount !== this.account.accountId) {
            throw new Error(
                `Cannot sign transaction as ${signingAccount} with Account for ${this.account.accountId}`,
            );
        }
    }
    ...
    async signAndSend<T extends SerializedReturnValue>(
        ...
        const { signedTransaction } = await this.toSignedTransaction({
            publicKey:
                this.publicKey ||
                (await this.account.getSigner().getPublicKey()),
            blockHash:
                this.blockHash ||
                (await this.account.provider.viewBlock(blockReference))?.header
                    ?.hash,
        });

        const outcome =
            await this.account.provider.sendTransaction(signedTransaction);
        return {
            outcome,
