 ...
 * 
 * In order to test nftTransferCall, you will need to deploy a contract that
 * implements the receiving function. One is provided by `near-sdk-rs` and is built
 * alongside the NFT contract:
 * https://github.com/near/near-sdk-rs/tree/master/examples/non-fungible-token/test-token-receiver
 * 
 * - create a new test account for the receiver contract: hereafter referred to as TRANSFER_CONTRACT_ID
 * - deploy with init call
 *   - `near contract deploy <TRANSFER_CONTRACT_ID> use-file res/token_receiver.wasm with-init-call new json-args '{"non_fungible_token_account_id":"<CONTRACT_ID>"}' prepaid-gas '100.0 Tgas' attached-deposit '0 NEAR' network-config testnet sign-with-keychain send`
 * - run this script with an additional argument for the transfer contract address:
 *   - `pnpm nonFungibleTokenStandard -- <CONTRACT_ID> <ACCOUNT_ID> <RECEIVER_ID> <TRANSFER_CONTRACT_ID>`
 */
...
export default async function nonFungibleTokenStandard(
  ...
  if (!contractAddress || !accountId || !receiverId) {
    ...
    if (transferContractAddress) {  
        await nftTransferCallInvoke(contractAddress, accountId, receiverId, "1");
    } else {
      console.log(chalk`{white skipping nftTransferCallInvoke since no transfer contract address was provided}`);
    }
}
...
export async function nftTransferCallInvoke(
    contractAddress: string,
    accountId: string,
    ...
    const { outcome } = await nftTransferCall({
      receiver: contractAddress,
      sender: accountId,
        args: {
            receiver_id: receiverAddress,
            token_id: tokenId,
            msg: "return-it-now", // defined in the receiver contract, returns the token immediately
        },
        deps: {
            rpcProvider,
            signer,
    ...
    const output = chalk`
{white ------------------------------------------------------------------------ }
{bold.green RESULTS} {white nft_transfer_call}
{white ------------------------------------------------------------------------ }
{bold.white Transaction Hash} {white |} {bold.yellow ${outcome.transaction.hash}}
{white ------------------------------------------------------------------------ }
  `;
