...
describe('rpc-errors', () => {
    ...
    test('test parse error object', async () => {
        ...
        const failure = JSON.parse(errorStr).status.Failure;
        const error = parseRpcError(failure);
    ...
    test('test getErrorTypeFromErrorMessage', () => {
        ...
        // @ts-expect-error test input
        expect(getErrorTypeFromErrorMessage(err1)).toEqual(
            'AccountDoesNotExist',
        );
        // @ts-expect-error test input
        expect(getErrorTypeFromErrorMessage(err2)).toEqual(
            'AccountDoesNotExist',
        );
        // @ts-expect-error test input
        expect(getErrorTypeFromErrorMessage(err3)).toEqual(
